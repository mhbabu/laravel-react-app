...
class TaskCategoryController extends Controller{
    ...
    public function index(Request $request){
        ...
        if(isset($request->order))
            $query->orderBy($request->order['sortColumn'], $request->order['sort']);

        $taskCategories = $query->paginate($request->input('rows', 10));

        return response(['data' => $taskCategories ], Response::HTTP_OK);
